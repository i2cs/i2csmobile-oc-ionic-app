...
angular.module('shop.module').directive('itemTemplate', function () {
    return {
        ...
        controller: ['$scope', '$state', '$ionicPopup', '$ionicLoading', '$ionicTabsDelegate', 'locale', 'ShopService', 'REWARDS_ENABLED', function ($scope, $state, $ionicPopup, $ionicLoading, $ionicTabsDelegate, locale, ShopService, REWARDS_ENABLED) {
            $scope.reward = REWARDS_ENABLED;
            $scope.item.reward = parseInt($scope.item.reward);
