...
angular
    .module('cart.module')
    .controller('CartHomeCtrl', function ($scope, $rootScope, $ionicLoading, CartService, COUPONS_ENABLED, REWARDS_ENABLED) {
        var vm = this;
        $scope.items = [];
        ...
        $scope.loadCart = function () {
            ...
            CartService.GetCart().then(function (data) {
                ...
                if ($scope.rewards_enabled && $rootScope.userLoggedIn())
                    $scope.getAvailableRewards();
            }, function (data) {
                $ionicLoading.hide();
                $scope.$broadcast('scroll.refreshComplete');
                $scope.dataLoaded = true;
        ...
        $scope.getAvailableRewards = function(){
            CartService.GetAvailableRewards().then(function (data) {
                $scope.maxRewards = data.maximum;
            });
        }
        ...
        $scope.submitRewards = function (reward) {
            $ionicLoading.show();

            CartService.ApplyRewards(reward).then(function (data) {
                $scope.loadCart();
                $ionicLoading.hide();
                alert(data.success);
            }, function (data) {
...
angular
    .module('cart.module')
    .controller('CartCheckoutCtrl', function ($scope, $rootScope, $state, $localStorage, $ionicTabsDelegate, $ionicModal, $ionicLoading, $ionicPopup, locale, appService, CartService) {
        var vm = this;
        ...
        $scope.savePersonalInfo = function () {
            if ($scope.forms.checkoutForm.$invalid) {
                $ionicPopup.alert({
                    title: locale.getString('cart.warning_fields_missing'),
                    cssClass: 'desc-popup',
                    scope: $scope,
